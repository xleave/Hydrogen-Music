...
use crate::library_index::{CachedTrack, Fingerprint};
...
pub fn scan(
    ...
    let cache_entries = crate::library_index::load(index_path).unwrap_or_else(|error| {
...
fn persist_index(path: &Path, cache: &ScanCache, prune_stale: bool) -> Result<(), String> {
    ...
    crate::library_index::persist(path, updates, stale)
}
