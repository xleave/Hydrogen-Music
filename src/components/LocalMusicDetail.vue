    ...
    <div class="local-music-container">
      ...
      <div id="local-list" class="local-music-list">
        ...
        <RecycleScroller
          ...
          v-slot="{ item, index }"
        >
        <div class="list-item" :class="{'list-item-playing': songId === item.id}" @dblclick="play(item.id, index)" @contextmenu="openMenu($event,item)">
          ...
          <div class="item-other">
            <div class="item-author">
              <span class="item-singer" v-if="item.common.artists && item.common.artists[0] !== '其他'" v-for="(singer, index) in item.common.artists" :key="singer">{{singer}}{{index === item.common.artists.length -1 ? '' : '/'}}</span>
            </div>
