...
<template>
    <div class="list-item" @click.stop="showFiles(item)" :key="item.id || item.dirPath" :class="{'list-item-open': isExpanded(item) && item.children.length !== 0, 'list-item-selected': router.currentRoute.value.query.id === (item.id || item.dirPath)}" v-for="item in props.folderChildren">
        <div class="folder" >
