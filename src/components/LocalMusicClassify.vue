<script setup>
  ...
  const showFiles = (item) => {
    if(item.type === 'album') router.push({ name: 'localAlbum', params: { id: item.id } })
    if(item.type === 'artist') router.push({ name: 'localArtist', params: { id: item.id } })
  }
</script>
...
<template>
  ...
  >
    ...
    <template #after>
      <div class="classify-end-space"></div>
    </template>
  </RecycleScroller>
</template>
...
<style scoped lang="scss">
  .classify-list{
    height: 100%;
    overflow: auto;
    &::-webkit-scrollbar { width: 5px; height: 10px; background-color: rgba(0, 0, 0, 0); }
    &::-webkit-scrollbar-thumb { background-color: rgba(0, 0, 0, 0.0); }
    &::-webkit-scrollbar-track { display: none; }
    &:hover::-webkit-scrollbar-thumb{ background-color: rgba(0, 0, 0, 0.04); }
    .list-item{
        height: 76Px;
        ...
        &:hover{ cursor: pointer; &::after{ transform: translateX(0); } }
        .item-img{
          margin-right: 10Px;
          padding: 5Px;
          width: 50Px;
    ...
    .list-item-selected{ &::after{ transform: translateX(0); } }
    .classify-end-space{ height: 15Px; }
  }
</style>
